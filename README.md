...
После клонирования создайте файл `.env` **на основе** `.env.example`:

```bash
cp .env.example .env
```

Далее при необходимости измените значения переменных в `.env`:
...
* `VITE_USE_MOCK=true` — использовать локальные тестовые данные из `src/mocks/news.json` (режим по умолчанию).
* `VITE_USE_MOCK=false` — использовать реальное API новостей (требуется `VITE_NEWS_API_KEY`).
* `VITE_NEWS_API_KEY` — ключ доступа к новостному API (например, NewsAPI.org).

Если файл `.env` не создан, приложение работает в режиме mock-данных.
